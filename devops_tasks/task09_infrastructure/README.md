...
- /docker - тут лежат конфигурационные файлы ```Docker``` и ```alert-receiver``` как часть реализации ```alertmanager```.
- /k8s - тут лежат конфигурационные файлы ```Kubernetes```
- /systemd - тут лежит unit файл .service для работы приложения как системного сервиса
- /screenshots - тут лежат скриншоты, которые используются ниже в отчете при описании хода выполнения работы
